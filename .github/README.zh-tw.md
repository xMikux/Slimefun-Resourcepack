...
[![All Contributors][all-contributors-badge]](#-貢獻者們)
...
| 模組 | 介紹 |
| --- | --- |
| [CIT Resewn][modrinth-cit] | 支援更多自訂物品功能<br /> (例如 `盔甲外層`) |
| [Slimefun Essentials][modrinth-slimefun-essentials] | 支援 EMI、REI、JEI 合成表查看與更多！<br />詳細請查看模組介紹。 |
...
在 [Modrinth][modrinth] 上下載資源包，並在 Minecraft 中載入。

> ℹ️ 注意<br />
> 伺服器所有者必須安裝 `item-models.yml` 在伺服器上來配對物品。
...
- item-models 安裝方式
  1. 在 [Modrinth][modrinth] 上下載 `item-models.zip`。<br />
     你可以在 `Version` 中的特定版本資源中找到 <br/>
     內有三個檔案可以被下載。
  2. 解壓縮 `item-models.zip` 來得到 `item-models.yml`。
  3. 放置其在 `plugins/Slimefun/` 中。

- ItemsAdder 安裝方式
  1. 在 [Modrinth][modrinth] 上下載 `ia-addons.zip`<br />
     你可以在 `Version` 中的特定版本資源中找到 <br/>
     內有三個檔案可以被下載。
  2. 解壓縮 `ia-addons.zip` 並放置 `contents` 在 `plugins/ItemsAdder` 中。
  3. 輸入 `/iareload` 來載入新的紋理。 <br />
     `/iazip` 來壓縮新的材質。
...
- **主版號**: 不會變動（除非我想要將整個 ID 範圍重作一次）
- **次版號**: 當變更或新增紋理時
- **修訂號**: 當修正、更新位置、版本等等

## 📝 內容與資訊

目前仍有許多紋理缺失！歡迎貢獻補充更多紋理！
...
- ExtraGear 盔甲
- Gastronomicon 盔甲
- GeneticChickengineering 小雞

</details>

<details>
<summary>模型 ID 範圍</summary>
...
感謝這些很棒的人（[表情符號代表][all-contributors-emoji-key]）：
...
本專案遵循 [all-contributors][all-contributors] 規範定義。
歡迎任何形式的貢獻！
